...
public class SteamUserClient {
    ...
    /**
     * Настраивает стратегию повторных попыток для запроса
     */
    ...
    @PostConstruct
    public void init() {
        retryBackoffSpec = reactiveRetryStrategy.doFixedDelayRetry(props.retryAttempts(),
     ...
     *
     * @param steamId
     *         Steam ID пользователя, информацию о котором нужно получить
     *
     * @return {@link SteamPlayerResponse}, содержащий данные о пользователе
     *
     * @throws RuntimeException
     *         если запрос к Steam API завершился неудачно
     */
    public Mono<SteamPlayerResponse> fetchPlayerSummaries(String steamId) {
        log.info("Fetching player summary for steamId={}", steamId);
        ...
        return steamWebClient.get()
                             ...
                             .doOnSuccess(resp -> log.info("Player summary response: {}", resp))
                             .doOnError(error -> log.error(
                                     "Error fetching player summaries for steamId={}",
                                     steamId,
                                     error
                             ))
                             .onErrorMap(error -> new GameRecommenderException(
                                     ErrorType.STEAM_API_PLAYER_SUMMARY_ERROR,
                                     steamId
                             ));
    }
     ...
     *         указывает, нужно ли включать дополнительную информацию об играх
     * @param includePlayedFreeGames
     *         указывает, нужно ли включать бесплатные игры, в которые играл пользователь
     *
     ...
     *         если запрос к Steam API завершился неудачно
     */
    public Mono<SteamOwnedGamesResponse> fetchOwnedGames(
            String steamId,
        ...
        return steamWebClient.get()
                             ...
                             .retryWhen(retryBackoffSpec)
                             .doOnSuccess(resp -> log.info("Owned games response: {}", resp))
                             .doOnError(error -> log.error(
                                     "Error fetching owned games for steamId={}",
                                     steamId,
                                     error
                             ))
                             .onErrorMap(e -> new GameRecommenderException(
                                     ErrorType.STEAM_API_FETCH_OWNED_GAMES_ERROR,
                                     steamId
                             ));
    }
